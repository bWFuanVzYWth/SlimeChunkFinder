...
pub struct SlimeChunkLut {
    lut: Box<[u32; LUT_LENGTH]>,
}

impl SlimeChunkLut {
    pub fn new() -> Self {
        Self {
            lut: (0..(LUT_LENGTH / BLOCK_LENGTH) as u32)
                .into_par_iter()
                .map(|chunk_i| {
                    let seed_base = chunk_i * (SEED_BITWISE * BLOCK_LENGTH) as u32;
                    Self::process_block(seed_base)
                })
                .collect::<Vec<_>>()
                .concat()
                .try_into()
    ...
    fn process_block(seed_base: u32) -> [u32; BLOCK_LENGTH] {
        const M: u32 = 397;
    ...
    pub const fn is_slime_chunk(&self, seed: u32) -> bool {
